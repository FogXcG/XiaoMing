...
def _worker_protocol_context_item() -> dict:
    return {
        ...
        "content": (
            ...
            "Before inspecting files, writing files, running commands, or reporting progress, decide whether any available skill applies. If a skill applies, call skill with action=load before acting. If you skip an obvious skill, state the reason briefly.\n"
            ...
            "For skill installation, install only after the source is known. Use the native skill tool with action=install instead of recreating installer behavior with shell, git clone, curl, mkdir, cp, or write_file. If a skill install source is unknown, load find-skills with skill(action=load) to discover candidates or ask the coordinator with talk for the GitHub URL or repo/path. Do not assume openai/skills is the default source.\n"
